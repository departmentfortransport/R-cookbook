...
Firstly, the **current year** is specified and the rounding function declared. This function is used because the default round function in R rounds upwards only if the number is even. 
...
The contents of the published table is read in (the last 10 years only), The adjusted data is rounded to the nearest whole number. The year field is changed to a character format so that it is displayed without the thousand separator comma.
...
Severity <- Last3Years %>% tidyr::gather(Fatal,SeriousUnadjusted,SeriousAdjusted,
...
Transposed <- Severity %>% tidyr::spread(YearLab, n) %>%
